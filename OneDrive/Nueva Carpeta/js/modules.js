...
class TurnoManager {
    ...
    static async reiniciarDatos() {
        ...
        if (!tieneEmpleadosConDatosDelMes) {
            console.log(`[TurnoManager.reiniciarDatos] ⭕ NO hay datos para ${AppState.currentMonth}/${AppState.currentYear}`);
        } else {
            console.log(`[TurnoManager.reiniciarDatos] ✅ Cuadrante tiene datos del mes actual`);
        }
        
        // ✅ ACTUALIZAR BOTÓN - ESPERANDO A QUE CARGUEN LOS DATOS
...
// =============================================
// ⚠️ DEPRECATED: LocationManager - Usa ConsolidadoLocalidades + LocalidadesManager (modular)
// Eliminado para evitar duplicación de código. Ver: js/localidades-manager.js y js/consolidado-localidades.js
// ⚠️ DEPRECATED: TurnoTypeManager - Usa ConsolidadoTurnos + TurnosTypesManager (modular)
// Eliminado para evitar duplicación de código. Ver: js/turnos-types-manager.js y js/consolidado-turnos.js
